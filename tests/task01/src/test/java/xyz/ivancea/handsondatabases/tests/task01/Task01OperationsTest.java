...
class Task01OperationsTest extends TaskOperationsTestCase<Task01Operations> {
    ...
    @Override
    protected Task01Operations createSolution(FileHelper fileHelper) {
        return new Task01SolutionAdapter(fileHelper);
    }

    @Override
    protected void doCheckImplementation(Task01Operations implementation) {
        implementation.store(42);
        implementation.read();
    }
}
